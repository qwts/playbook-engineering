...
directories today; installable skills live in their owner repos and are
listed under Available skills. A skill that later lands here is an
...
- **Home / index:** this file. Agents look here to learn what the fleet
  can install. `docs-gov` enrolls markdown that exists in this checkout,
  so an external skill is advertised by owner-repo URL, not by a dummy
  folder under `skills/`.
- **Implementation:** the owner repo linked from each Available skills
  entry. Install from that checkout or its documented distributor. Do not
  copy the skill into this repository.

## Available skills

These are installable. They do not live under this repository's `skills/`
tree.
...
- **[agent-bot](https://github.com/qwts/agent-bot-identity/tree/main/skills/agent-bot)**
  ...
  bot identity bootstrap, credential minting, verified commits, and
  Agent Spaces. Reference docs live in that skill's
  [references](https://github.com/qwts/agent-bot-identity/tree/main/skills/agent-bot/references)
  directory. This skill absorbed the old signed-commit skill.
- **[managed-machine](https://github.com/qwts/managed-machine/blob/main/skills/SKILL.md)**
  ...
  Mac bootstrap and update via the Homebrew formula.

The signed-commit skill used to live here and moved into agent-bot. A
machine that installed it from this repo has a dangling symlink; remove
it:
...
`SKILL.md` carries its own install line, and the harness gets a symlink
so a `git pull` here updates every machine, rather than copying and
drifting:
...
`SKILL.md`, or the Homebrew formula for managed-machine. Do not symlink
...
1. For a skill that lands in this home: `skills/<name>/SKILL.md` with
   ...
   an Available skills entry with that repo's URL; do not create a
   placeholder directory here.
