...
run `node tools/agent-bot/setup-worktree.mjs` from the harness's own
session-start hook (Claude Code: a `SessionStart` command hook in
`~/.claude/settings.json`) — it is idempotent and no-ops everywhere except an
unconfigured linked worktree — or run it once manually in the worktree.

**Agents working in a primary checkout** (e.g. Codex in a dedicated clone)
are outside the hook's guard *by design*, so identity there rides on
per-session env exports and silently reverts to `qwts` next session. When a
clone is de facto an agent workspace, pin its identity once:

```bash
BOT=qwts-codex-agent   # the App this clone authors as
BOT_UID=$(cat ~/.config/$BOT/bot-uid)
git config user.name "${BOT}[bot]"
git config user.email "${BOT_UID}+${BOT}[bot]@users.noreply.github.com"
git config commit.gpgsign false
git config --add credential.helper ''
git config --add credential.helper "!node $HOME/Code/playbook-engineering/tools/agent-bot/git-credential-bot.mjs $BOT"
```

Undo with `git config --unset-all user.name user.email` etc. if the human
takes the clone back.
