...
worktree — it is idempotent and no-ops everywhere except an unconfigured
linked worktree. (No per-harness session machinery: per ENG-0045 the model
carries zero tool-specific mechanisms, and the pre-commit guard turns a
missed hook into a loud error instead of a silent `qwts` commit.)
...
identity. The machine-wide `pre-commit` guard enforces the agent side of that
